...
module Seq: S = struct
  ...
  type 'a channel = { q : 'a Queue.t; w: ('a -> unit) Queue.t }
  ...
  let new_channel () = let c = { q=Queue.create (); w= Queue.create () } in c, c

  let rec get f continuation =
    match (Queue.is_empty f.q), (Queue.is_empty f.w) with

    (*The value queue is empty, keep the continuation for later*)
    | true, _ -> Queue.add continuation f.w

    (*The value queue is not empty, the funcion queue is empty*)
    | false, true -> continuation (Queue.take f.q)

  let put v f continuation =
    Queue.add v f.q;
    match (Queue.is_empty f.w) with
    | false -> enqueue (fun () -> (Queue.take f.w) (Queue.take f.q)); enqueue continuation
    | true -> enqueue continuation
